...
| L1 norm | | |the mean absolute difference | |
| L2 norm | | | the root mean squared error (RMSE) | |
| Baseline | | | A simple model that you are confident should perform reasonably well. | |
| overfitting | | | | |
| | | | | |
| | | | | |
| | | | | |
